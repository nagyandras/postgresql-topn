...
MODULES = topn
EXTENSION = topn
EXTVERSIONS = 2.0.0 2.1.0 2.2.0 2.2.1 2.3.0
...
SQLPP ?= cpp -undef -w -P -imacros $(shell $(PG_CONFIG) --includedir)/pg_config.h
...
$(EXTENSION)--2.2.1.sql: $(EXTENSION)--2.2.0.sql $(EXTENSION)--2.2.0--2.2.1.sql
	cat $^ > $@
$(EXTENSION)--2.3.0.sql: $(EXTENSION)--2.2.1.sql $(EXTENSION)--2.2.1--2.3.0.sql
	cat $^ > $@
